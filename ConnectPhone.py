...
def is_valid_ip(ip):
    # Proxy to OOP
    return ConfigurationManager._is_valid_ip(ip)

def load_config():
    # Proxy to OOP
    return global_config_mgr.load()

def save_config(config):
    # Proxy to OOP
    global_config_mgr._config_data = config
    global_config_mgr.save()
...
def get_macos_audio_devices():
    ...
    _cached_audio_devices = devices
    return _cached_audio_devices

def load_last_ip():
    return global_config_mgr.get("last_ip", global_config_mgr.DEFAULT_CONFIG["last_ip"])
...
def run_mirroring_flow(mode, config):
    ...
    if mode == 1:
        ...
        if cam_orientation == "portrait":
            ...
            rec_orient = "0"
        if disp_orient != "0":
            args.append(f"--display-orientation={disp_orient}")
            args.append(f"--record-orientation={rec_orient}")
